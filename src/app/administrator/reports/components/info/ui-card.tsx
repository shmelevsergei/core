...
const UiCard = ({ text, count, percent, link }: IUiCardProps) => {
    return (
        <Card
            className={cn(
                'flex items-center gap-10 w-full h-full py-3 px-3 bg-red-100 shadow-md border-none'
            )}
        >
            <CardHeader className={cn('max-w-[250px] p-0')}>
                <CardDescription
                    className={cn('text-base text-black font-medium')}
                >
                    {text}
                </CardDescription>
            </CardHeader>
