...
import {fetchAnketaOborotNotByDistributor} from "@/server/routs/fetchAnketaOborot";
...
const NotByFromDistributor = () => {
    ...
    useEffect(() => {
        ...
        setPercent((100 -((state.purchaseSto / state.totalSto) * 100)).toFixed(1))

    }, [state.totalSto, state.purchaseSto])


    return (
        <UiCard text={'Не закупают ни у одного дистрибьютора'} count={count} link={''} percent={+percent} />
    );
};
